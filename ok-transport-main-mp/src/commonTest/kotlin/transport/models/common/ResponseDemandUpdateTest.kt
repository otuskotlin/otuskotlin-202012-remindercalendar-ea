...
internal class ResponseDemandUpdateTest {
    @Test
    fun serializationResponseDemandUpdateTest() {
        val json = Json {
            prettyPrint = true
            serializersModule = SerializersModule {
                polymorphic(MpMessage::class) {
                    subclass(MpResponseDemandUpdate::class, MpResponseDemandUpdate.serializer())
                }
        ...
        val dto: MpMessage = MpResponseDemandUpdate(
        ...
        assertEquals("response-id", (deserializedDto as? MpResponseDemandUpdate)?.responseId)
    }
}